...
SELECT id, code, name, description, parent_id,
       CASE WHEN id IN (4,5,8) THEN 'INACTIVE' ELSE status END,
       level, CURRENT_TIMESTAMP - INTERVAL '60 days', FALSE
FROM spp_elements;

INSERT INTO spp_history (element_id, code, name, description, parent_id, status, level, valid_from, is_current)
SELECT id, code, name, description, parent_id, status, level, CURRENT_TIMESTAMP - INTERVAL '30 days', FALSE
FROM spp_elements;
...
INSERT INTO spp_history (element_id, code, name, description, parent_id, status, level, valid_from, is_current)
SELECT id, code, name, description, parent_id, status, level, CURRENT_TIMESTAMP - INTERVAL '10 days', TRUE
FROM spp_elements;
